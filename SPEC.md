...
`io.print` and `io.println` are format-backed macros. Their first argument is a
Rust-style literal template using `{}` placeholders and `{{` / `}}` escapes;
`print` writes exactly the formatted text and `println` performs one additional
newline write. The historical single-integer `(io.print value)` spelling keeps
its newline behavior as a compatibility form; use `(io.print "{}" value)` for
the new no-newline semantics. `print-string`, `print-str`, `print-newline`,
`print-error`, `print-bool`, `print-char`, and `print-float` retain their existing
behavior, as do the low-level borrowed `stdout-write` and `stderr-write` helpers.
