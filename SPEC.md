...
- Fixed-size, heterogeneous inline structural values. Layout is sequential
  with natural alignment per element, recursively through nested aggregates.
- Tuple literals and `tuple-ref` use that inline representation. Non-empty
  tuples can cross internal function boundaries as by-value parameters and
  returns; their target-specific transport is specified in section 11.

**Fixed array:** `(Array type size)`
- Size must be a compile-time constant.
- Fixed-array literals are inline structural values with recursive natural
...
lifetimes, call capability, or the Copy class of a concrete environment.
TypeLisp therefore uses checker-only facts rather than a source-visible
type-level capture marker. The checker attaches capture-lifetime, capability,
and environment Copy facts to lambda expressions and fact-preserving local
closure bindings. If a flow would erase a required fact, the flow is treated as
escaping and is rejected. The runtime closure descriptor ABI is unchanged.
...
For lambda literals and their checker-known local bindings, the checker also
retains a call-capability fact that is not present in `(-> ...)`:
...
Concrete environment Copy classification follows section 4.7.2. A closure
whose captures are all Copy may be copied through a local alias, ordinary
by-value call, return, or aggregate slot. When those captures include shared
references, every fact-preserving local copy retains the same lifetime and NLL
lending provenance; copying never lengthens the borrow or makes an escaping
flow legal.

A non-Copy closure may move once into a checker-known local alias or enclosing
local closure, and the source is invalid afterward. Returning, storing, or
passing it through a plain `(-> ...)` value would erase its concrete ownership
fact and therefore fails closed. Mutable/reusable and consuming capability
facts remain stricter: aliasing them would erase exclusive or one-shot call
semantics and is rejected. Lifetime facts survive Copy local aliases and nested
local closures whose complete use remains in the proven scope; all other
fact-erasing flows reject. Macro expansion and control-flow checks apply the
same rules deterministically.

Checker-recognized ownership-transfer operations may consume a direct
non-Copy lambda without widening it to a reusable plain function value. The
safe thread-spawn surface is the current example: it separately validates the
direct lambda and transfers each permitted capture exactly once.
...
- Named top-level and non-capturing function values. A plain `AstType.Func`
  shape is Copy only when no checker-known concrete closure fact overrides it.
...
not merely its plain `AstType.Func` shape. Capture lifetime, Copy, and call
...
- A descriptor for an all-Copy environment may itself be copied; the current
  immutable capture model permits copies to share that immutable environment
  storage. A descriptor with any non-Copy capture is move-only. Copying never
  performs a deep clone, allocates a replacement environment, duplicates
  cleanup ownership, or changes the C-facing function-pointer ABI.
...
The embedded-stdlib catalog has a checked, source-derived identity inventory.
...
catalog and a content-modified source control. Each native identity and
declared result kind must be observed, catalog fallback counters must remain
zero, the source control must observe every identity with zero native dispatch,
and all fixture assembly must be byte-identical. For `Module` and `Decls`
results that exact assembly comparison includes generated identity/content,
deduplication, and splice order. Malformed request/handle, authored diagnostic,
native abort, and fuel exhaustion controls must leave no partial committed
result; authored diagnostics must match the forced-source route.
...
| `tl_substring`, `tl_str_view`, `tl_string_concat`, `tl_string_concat3`, `tl_string_concat4`, `tl_string_concat5`, `tl_int_to_string` | Runtime-plan compatibility names; implementations are TypeLisp exports. `tl_int_to_string` is safe. The raw-pointer substring/concat entries and the unchecked borrowed-view ABI carrier are unsafe callables; compiler-generated and safe stdlib call paths establish their range, overflow, initialization, lifetime, and fresh-destination obligations. |
| `tl_array_fill8`, `tl_mem_fill8`, `tl_mem_copy_fwd`, `tl_array_zero` | Unsafe raw-memory compatibility names. Positive fills require aligned live writable qword storage; copy requires complete live initialized source and writable destination byte ranges and deliberately preserves forward-copy overlap behavior. Fill/copy non-positive counts access no memory. `tl_array_zero` requires a non-negative byte count and is only for fresh allocator backing storage because its virgin-arena path may skip physical writes; it is not a general clearing primitive. |
| `tl_atomic_i64_load_ptr`, `tl_atomic_i64_store_ptr`, `tl_atomic_i64_add_ptr`, `tl_atomic_i64_fetch_add_ptr`, `tl_atomic_i64_cas_ptr`, `tl_atomic_i32_load_ptr`, `tl_atomic_i32_store_ptr`, `tl_atomic_i32_add_ptr`, `tl_atomic_i32_fetch_add_ptr`, `tl_atomic_i32_cas_ptr` | Unsafe compatibility names; implementations are TypeLisp exports over the atomic memory-operation intrinsics. Callers must prove live, correctly aligned and initialized storage of the named width, a sufficient lifetime, and permitted concurrent access. The safe owning atomic-buffer API establishes those obligations internally. |
| `tl_oob_abort`, `tl_div_abort`, `tl_shift_abort`, general panic/OOM aborts, file/IO/process/fs helpers, env lookup, random seed, profile time, and CPU feature helpers | Not backend-owned; these are TypeLisp runtime-prelude exports, stdlib implementations, or direct platform bindings. Source-located abort entries that accept a compiler-emitted descriptor pointer are unsafe callables: the backend supplies a live aligned four-word descriptor and initialized path range. |
...
- Tooling: package builds with application-owned lockfiles and shallow direct
  ...
  comment blocks, full-document semantic tokens that merge current-buffer
  lexical spans with exact current semantic records under negotiated UTF-8 or
  UTF-16 positions, a deterministic multi-root workspace source/declaration index
...
- **Internal structural values.** Every non-empty finite Tuple or fixed
  `(Array T N)` has an inline representation laid out recursively with the
  natural alignment of each element. The TypeLisp internal ABI classifies a
  structural value of size at most 16 bytes one eightbyte at a time: an
  eightbyte containing only floating leaves is `SSE`; all other eightbytes are
  `INTEGER`. Nested tuples and arrays contribute their leaves recursively, so
  a value may have one `INTEGER` and one `SSE` eightbyte. Parameters consume
  the independent integer and SSE register banks shown above. If either bank
  cannot allocate every eightbyte required by one aggregate, register
  allocation is rolled back for that aggregate and the complete value falls
  back to the normal stack argument area; an aggregate is never split between
  registers and stack.
  Structural returns use the corresponding result-bank cursors independently:
  INTEGER eightbytes use `%rax` then `%rdx`, while SSE eightbytes use `%xmm0`
  then `%xmm1`; thus a mixed INTEGER/SSE result uses `%rax` and `%xmm0`. A
  value larger than 16 bytes is `MEMORY`: its parameter is passed in the stack
  argument area and its return uses a leading caller-owned sret pointer; the
  callee returns that pointer in `%rax`.
- Structural values are copied shallowly by representation. Passing or
  returning one never inserts an implicit clone, destructor, or drop.
- Empty tuples and zero-sized fixed arrays are rejected in internal parameter
  and return positions with the stable target-specific, source-located lower
  diagnostic: they have no transport representation. This rule concerns only
  the TypeLisp internal ABI; it does not broaden or change extern C aggregate
  eligibility or contracts.
...
- Scalar `f64` and `f32` both use the float (`%xmm`) registers: `f64` moves with
  `movsd`, `f32` with single-precision `movss`.
- Return value: `%rax` (integer), `%xmm0` (float, `f64` or `f32`).
- **Internal structural values.** Non-empty finite Tuple and fixed
  `(Array T N)` values use the same recursive, naturally aligned inline layout
  described above. A structural value of size 1, 2, 4, or 8 bytes is passed by
  value in the integer register/value transport (`%rcx`, `%rdx`, `%r8`, or
  `%r9`, then the stack area after shadow space). Every other parameter size
  is passed indirectly through a pointer to a private caller-created
  temporary. A return of 1, 2, 4, or 8 bytes uses `%rax`; every other
  structural return size uses a leading caller-owned sret pointer and returns
  that pointer in `%rax`.
- Structural values are copied shallowly by representation. Passing or
  returning one never inserts an implicit clone, destructor, or drop.
- Empty tuples and zero-sized fixed arrays are rejected in internal parameter
