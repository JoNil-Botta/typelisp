/* benchmarks/spmd_map/baseline.c - clang C baseline for spmd_map (#1125).
 *
 * Equivalent to benchmarks/spmd_map/bench.tl: run `reps` passes of an
 * elementwise map `out[i] = a[i] + b[i] + r` over a fixed-size array, fold one
 ...
 * TypeLisp i64 `+`, so both programs produce the identical exit code on a given
 * host.
 *
