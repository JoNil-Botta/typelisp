...
# scripts/fetch-stage0.sh), compile src/cli.tl to assembly and assemble +
...
echo "[build-stage0] compile src/cli.tl with seed ($NL_BOOTSTRAP_TARGET)"
if ! run_with_heartbeat_capture "compile cli.tl" "$COMPILE_STDOUT" "$COMPILE_STDERR" \
    "$SEED" compile src/cli.tl -o "$ASM" \
    --target "$NL_BOOTSTRAP_TARGET" \
    $(native_target_cfg_args) \
    --stdlib-root stdlib --stdlib-root src --opt-level 1; then
    echo "[build-stage0] seed compiler failed while compiling src/cli.tl" >&2
...
[ -s "$ASM" ] || {
    echo "[build-stage0] seed did not emit assembly for src/cli.tl" >&2
    exit 1
}
