...
if grep -Eq '\(Array[[:space:]]+[A-Za-z_][A-Za-z0-9_.]*\)' "$BENCH_SOURCE"; then
    echo "perfbench_stores: public unsized Array compatibility form remains" >&2
    exit 1
fi
if grep -Fq '__tl_dyn-array' "$BENCH_SOURCE"; then
    echo "perfbench_stores: caller-authored private dynamic array remains" >&2
    exit 1
fi
if grep -Fq '(import stdlib.array)' "$BENCH_SOURCE" ||
   grep -Fq '(array.make-array ' "$BENCH_SOURCE"; then
    echo "perfbench_stores: benchmark still owns compatibility array backing" >&2
    exit 1
fi
for required_surface in \
    ...
    '[values : f32v.Vec (stores-buffer (+ max-count 1))]'; do
    grep -Fq "$required_surface" "$BENCH_SOURCE" || {
        echo "perfbench_stores: missing Vec/Slice surface: $required_surface" >&2
        exit 1
    }
done
grep -Fq '(while (< repetition 100)' "$BENCH_SOURCE" || {
    echo "perfbench_stores: missing 100-call measured loop" >&2
    exit 1
}
view_count=$(grep -Ec '\(f32v\.view(-mut)?[[:space:]]' "$BENCH_SOURCE" || true)
[ "$view_count" -eq 3 ] || {
    echo "perfbench_stores: expected three persistent Vec views, found $view_count" >&2
    exit 1
}
awk '
    ...
    END { exit bad ? 1 : 0 }
' "$BENCH_SOURCE" || {
    echo "perfbench_stores: Vec view construction entered the measured loop" >&2
    exit 1
}
