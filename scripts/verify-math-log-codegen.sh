...
# Verify that freestanding logarithm functions stay allocation/libm-free and
...
WORKDIR="$ROOT/target/exp/math-log-codegen"
rm -rf "$WORKDIR"
mkdir -p "$WORKDIR"
...
verify_assembly() {
    ...
    if grep -Ei '^[[:space:]]*\.(extern|globl?)[[:space:]]+_?logf?([[:space:]]|$)' "$assembly" >/dev/null; then
        fail "$target assembly references an external logarithm function"
    fi
    if grep -Ei '^[[:space:]]*(fyl2x|fyl2xp1|fldln2|v?fmadd[a-z0-9]*|v?fmsub[a-z0-9]*|v?fnmadd[a-z0-9]*|v?fnmsub[a-z0-9]*)[[:space:]]' "$assembly" >/dev/null; then
