...
sh "$ROOT/tests/public-tools/test-result-checks.sh"
...
run_backend_mode_exec avx512 avx512
...
run_spmd_exec_mode avx512 avx512
else
...
if [ "$HAS_INSPECT_COMMAND" -eq 1 ]; then
    ...
    assert_package_surface_inspect() {
        ...
        assert_contains "$out" "surface-frontend-ast: present schema-version=10 bytes="
        ...
        assert_not_contains "$out" "surface-frontend-ast: present schema-version=10 bytes=0"
