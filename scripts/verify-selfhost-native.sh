...
verify_compiler_driver_box_list() {
    _driver=$1
    _dir="$WORKDIR/compiler-driver/box-list"
    mkdir -p "$_dir"
    _src="$_dir/input.tl"
    _asm="$_dir/output.s"
    cat > "$_src" <<'EOF'
(defenum IntList (Nil) (Cons i64 (Box IntList)))

(define (sum [xs : IntList]) : i64
  (match xs
    [Nil 0]
    [(Cons head tail) (+ head (sum (box-get tail)))]))

(define (main) : i64
  (let
    [tail : IntList (Cons 2 (box Nil))]
    [mid : IntList (Cons 30 (box tail))]
    [head : IntList (Cons 10 (box mid))]
    (sum head)))
EOF

    echo "[selfhost-native] compiler_driver recursive Box list"
    run_compiler_driver "$_driver" compiler-driver-box-list "$_src" "$_asm"
    assert_contains "$_asm" "call tl_alloc" compiler-driver-box-list
    assemble_link_run_asm compiler-driver-box-list "$_asm" 42 - - 1
}
...
verify_compiler_driver_box_list "$DRIVER"
verify_emit_printed_program
verify_parse_printed_program
verify_eval_driver_errors
