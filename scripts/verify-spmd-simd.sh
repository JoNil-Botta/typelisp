...
verify_byte_shift_shape() {
    _mode=$1
    compile_spmd_mode tests/spmd/byte_shift_value_types.tl "$_mode"
    _tag=tests_spmd_byte_shift_value_types_tl
    _asm="$WORKDIR/$_tag.$_mode.compile.s"
    if [ "$mode_code" != 0 ]; then
        echo "[spmd-simd] byte-shift $_mode shape compile failed:" >&2
        sed 's/^/    /' "$mode_err" >&2
        echo "tests/spmd/byte_shift_value_types.tl $_mode (shape compile)" >> "$FAILURES"
        return
    fi
    for lane in i8 u8; do
        _func="$WORKDIR/$_tag.$_mode.$lane.s"
        _gang="$WORKDIR/$_tag.$_mode.$lane.gang.s"
        sed -n \
            "/^_tl_byte_shift_value_types_run_direct_${lane}:/,/^_tl_.*:/p" \
            "$_asm" > "$_func"
        sed -n '/foreach_avx2_body/,/foreach_tail_header/p' \
            "$_func" > "$_gang"
        if [ ! -s "$_gang" ]; then
            echo "[spmd-simd] byte-shift $_mode missing direct-$lane SIMD gang" >&2
            echo "tests/spmd/byte_shift_value_types.tl $_mode direct-$lane (missing gang)" >> "$FAILURES"
            continue
        fi
        if [ "$lane" = i8 ]; then
            _widen=vpmovsxbd
        ...
        for opcode in "$_widen" vpsllvd "$_right" vpackusdw vpackuswb vpermd; do
            if ! grep -F -- "$opcode" "$_gang" > /dev/null; then
                echo "[spmd-simd] byte-shift $_mode direct-$lane missing $opcode" >&2
                echo "tests/spmd/byte_shift_value_types.tl $_mode direct-$lane (missing $opcode)" >> "$FAILURES"
            fi
        done
        if ! grep -F -- "call tl_shift_abort" "$_gang" > /dev/null; then
            echo "[spmd-simd] byte-shift $_mode direct-$lane lacks an active-lane trap guard" >&2
            echo "tests/spmd/byte_shift_value_types.tl $_mode direct-$lane (missing trap guard)" >> "$FAILURES"
        fi
        if grep -E -- '^[[:space:]]+(salb|sarb|shrb)[[:space:]]' "$_gang" > /dev/null; then
            echo "[spmd-simd] byte-shift $_mode direct-$lane scalarized inside the SIMD gang" >&2
            echo "tests/spmd/byte_shift_value_types.tl $_mode direct-$lane (scalar shift)" >> "$FAILURES"
        fi
        _instructions=$(grep -E -c '^[[:space:]]+[a-zA-Z]' "$_gang" || true)
        echo "[spmd-simd] byte-shift $_mode direct-$lane SIMD gang -> $_instructions instructions"
    done
}
