...
| `src/ir.rs` | 8 | Instruction effects, runtime helper effect classification, optimizer conservatism predicates, vector/mask display, tail masks, and tail-call pretty printing. | #1043 migrates IR structural assertions into selfhost IR/type tests and optimizer/lowerer smoke coverage. | Partial |
...
| `src/ctfe.rs` | 2 | CTFE type literal evaluation and type-value comparison. | #1043 migrates CTFE assertions into `compiler_ctfe.tl` self-tests and specialization/typechecker smoke cases. | Partial |
