...
| `tests/cli.rs` | Public CLI usage/errors and behavior for compile/check/build/run/fmt/doc/doc-test/debug/test/package-adjacent flows, LSP, REPL, and selfhost REPL. | `scripts/verify-public-tools.sh` now covers representative public CLI, docs, LSP, REPL, formatter, inline test runner, package, SPEC, and diagnostic cases; `doc --test` and `test` now route through selfhost TypeLisp drivers; remaining embedded edge cases still need migration. | Partial |
