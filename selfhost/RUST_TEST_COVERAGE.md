...
| `tests/cli.rs` | Public CLI usage/errors and behavior for compile/check/build/run/fmt/doc/doc-test/debug/package-adjacent flows, embedded stdlib fallback, LSP, REPL, and selfhost REPL. | `scripts/verify-public-tools.sh` drives the `tests/public-tools/` corpus (`run-corpus.py`) which covers public REPL, public LSP diagnostics/lifecycle, selfhost REPL/LSP frame behavior across 44 fixture files, the Linux selfhost build/run tool execution path (#1018), public formatter/doc/doctest pass/fail/malformed/expected-error behavior through the selfhost drivers (#845/#1019), direct selfhost-test-driver inline-harness execution accepting an opt-level flag and its explicit Windows-unsupported diagnostic (#1401, temporary Rust witness for the selfhost/test.tl + selfhost/build_run_core.tl no-Rust inline runner), and direct `selfhost/build.tl` package-build parity for manifest discovery, path dependencies, diagnostics, and option handling (#1339). `scripts/check-stage1-wrapper.sh` now covers top-level stage1 wrapper package-build routing for `--manifest-path`, upward discovery, and missing-manifest diagnostics (#1330). The remaining Rust CLI tests added for #1019 pin public-boundary behavior that the shell corpus cannot cheaply simulate: fmt/doc spaced paths on Windows, isolated missing selfhost formatter-driver diagnostics, missing assembler diagnostics, and doc-test CRLF normalization while the Rust launcher still fronts selfhost/run.tl. Embedded stdlib fallback CLI behavior is temporary Rust stage0 coverage for #1507; the no-Rust path is the bundled stdlib installed with the stage1 wrapper and the #949/#1200 release/capability gates. Broader selfhost public execution parity remains under #787/#795 and the remaining public command cutovers. | Partial |
