...
- #1455 adds selfhost backend emit-shape parity for checked div/mod/shift trap
  branches, `tl_div_abort` / `tl_shift_abort` calls, helper labels, and helper
  data through `compiler-backend-checked-trap-emit-shape-ok?`.
...
| `tests/selfhost_optimizer_aggregate.rs` | Selfhost optimizer stack-promotes non-escaping local aggregate storage at `--opt-level 2` (#1489): a fixture compiled through `selfhost/compile.tl` shows the aggregate slot rewritten to stack/scalar form rather than a heap allocation. | Covered by the selfhost optimizer self-tests in `selfhost/compiler_optimize_smoke.tl` (`compiler-optimize-aggregate-candidate-self-test` and related) run by `scripts/verify-selfhost-compile-manifest.sh`; Rust witness can be deleted with #1043/#795. | Covered |
