...
| --- | ---: | --- | --- | --- |
| `src/typechecker.rs` | 308 | Type rules for scalars, strings, arrays, structs, enums, tuples, functions, lambdas, patterns, matches, builtins, regions, comptime/type-valued parameters, diagnostics, loops, and SPMD reduce. | #1042 migrates these into typechecker module self-tests, inline TypeLisp tests, and accepted/rejected selfhost corpus cases; #947 owns autodiscovery when inline tests are used. | Partial |
...
| `src/lexer.rs` | 9 | Basic tokens, type tokens, single/multi-line spans, char literals, escapes, named characters, and unknown named-character errors. | #1041 migrates lexer assertions into `lex.tl` self-tests, inline tests, and parser smoke coverage. | Partial |
...
| `src/backend/regalloc.rs` | 5 | Register allocation eligibility, live-after-call exclusions, deterministic spills, vector/mask exclusions, and System V versus Windows clobber models. | #1044 migrates regalloc/liveness assertions into `compiler-regalloc-self-test`, backend smoke coverage, and platform-specific no-Rust checks. | Partial |
...
| `tests/cli.rs` | Public CLI usage/errors and behavior for compile/check/build/run/fmt/doc/doc-test/debug/package-adjacent flows, LSP, REPL, and selfhost REPL. | `scripts/verify-public-tools.sh` now covers representative public CLI, command matrices, check diagnostics, build/run behavior, host-action plans, docs, LSP, REPL, formatter, package, SPEC, and diagnostic cases; doc/doc-test edge cases from `tests/cli.rs` are covered by public fixtures. Remaining package-specific migration belongs to #1077, deeper REPL/LSP transcript coverage belongs to #1080, and selfhost public execution parity remains blocked by #787/#1018/#1019. | Partial |
