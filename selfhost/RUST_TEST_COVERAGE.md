...
| `tests/tl_doc_test_compile.rs` | Selfhost doctest extractor and policy fixtures compile. | `doc_test.tl` and `doc_test_smoke.tl` exist, and public `doc --test` now runs through `selfhost/doc.tl`; no-Rust compile manifest is owned by #846 and CLI harness retirement by #845. | Partial |
