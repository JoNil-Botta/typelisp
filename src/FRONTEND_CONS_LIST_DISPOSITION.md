...
| `AstParamList` | `src/compiler_ast_types.tl` | generated vector family (`ast-param-list-*`) | Ordered params; indexed traversal during bind/check/lower. | Hot in function-heavy files. | `converted-to-generated-vector` | #4182. |
| `AstMacroParamList` | `src/compiler_ast_types.tl` | generated vector family (`ast-macro-param-list-*`) | Ordered macro params; indexed traversal for arity/type checks. | Moderate. | `converted-to-generated-vector` | #4182. |
| `AstFieldDefList` | `src/compiler_ast_types.tl:285` | generated vector family (`ast-field-def-list-*`) | Ordered fields; layout offset/index scans. | Moderate/hot for aggregate-heavy code. | `converted-to-generated-vector` | #3427. |
| `AstExternLinkInputList` | `src/compiler_ast_types.tl:307` | `AstExternLinkInputList` vector family | Ordered linker metadata; mutable parser accumulation and borrowed traversal preserve source order without copy-on-append. | Cold. | `migrated-vector` | #4186. |
...
| `AstLetBindingList` | `src/compiler_ast_types.tl:359` | `AstLetBindingList` vector family | Ordered sequential bindings; bind/check/lower in order. | Hot. | `converted-to-generated-vector` | #4185. |
| `AstResourceBindingList` | `src/compiler_ast_types.tl:366` | `AstResourceBindingList` vector family | Ordered resource cleanup bindings. | Cold/moderate. | `converted-to-generated-vector` | #4185. |
...
| `TcStringList` | `src/compiler_typecheck_core.tl` | allocation-free `Nil` / `One`, then `Dense` snapshots over append-only string nodes | Bound-name sets plus ordered cleanup, cycle, purity, and transfer paths; retained parents and siblings share immutable nodes. | Frequent; usually tiny, with wide macro/body outliers. | `migrated-dense-persistent-spine` | #5937. |
| `TcNameIdList` | `src/compiler_typecheck_core.tl` | allocation-free `Nil` / `One`, then `Dense` snapshots over append-only i64 nodes | Name/lifetime/region sets and ordered recursion stacks; deduplicating `add` and duplicate-preserving `cons` share storage. | Hot across type, move, borrow, SPMD, and macro checks. | `migrated-dense-persistent-spine` | #5937. |
| `TcPoisonNameList` | `src/compiler_typecheck_core.tl` | immutable right-sized dense snapshots | Ordered poisoned-declaration rebinding; append-order iteration preserves the former oldest-to-newest bind order and each error-path update retains independent storage. | Error recovery; normally empty and only grows after diagnostics. | `migrated-dense-snapshot` | #5937. |
...
| `TcLifetimeSubst` | `src/compiler_typecheck_core.tl` | allocation-free `Nil` / `One`, then `Dense` snapshots over append-only `(formal, actual, parent)` nodes | Functional lifetime-inference substitutions; retained roots and retry siblings share immutable nodes while duplicate/conflict checks remain lookup-based. | Moderate; normally tiny, with wide-signature outliers. | `migrated-dense-persistent-spine` | #5931. |
...
| `TcReprCFieldLayoutList` | `src/compiler_typecheck_core.tl:880` | `defstruct` over one right-sized field array | Ordered field layout metadata. | Layout/C ABI paths. | `migrated-bespoke-dense-builder` | #5938. |
| `TcInlineFieldLayoutList` | `src/compiler_typecheck_core.tl:893` | `defstruct` over one cache-owned right-sized field array | Ordered inline field metadata; indexed offset scans. | Layout/typecheck/lower paths. | `migrated-bespoke-dense-builder` | #5938. |
| `TcInlinePayloadLayoutList` | `src/compiler_typecheck_core.tl:899` | `defstruct` over one cache-owned right-sized payload array | Ordered enum payload metadata. | Layout/typecheck/lower paths. | `migrated-bespoke-dense-builder` | #5938. |
| `TcInlineVariantLayoutList` | `src/compiler_typecheck_core.tl:905` | `defstruct` over one cache-owned right-sized variant array | Ordered variant layout metadata. | Layout/typecheck/lower paths. | `migrated-bespoke-dense-builder` | #5938. |
| `TcStdlibFieldSpecList` | `src/compiler_typecheck_core.tl:3115` | `Nil` / `Cons` | Tiny expected-field spec table. | Cold. | `keep-persistent-cons` | #3429 may confirm no conversion. |
| `TcStdlibVariantSpecList` | `src/compiler_typecheck_core.tl:3119` | `Nil` / `Cons` | Tiny expected-variant spec table. | Cold. | `keep-persistent-cons` | #3429 may confirm no conversion. |
| `TcMovedSet` | `src/compiler_typecheck_core.tl:25321` | `defstruct` over a generated String/i64 hashmap | Dense hash-map-backed move/borrow set, not a cons enum. | Hot move/borrow checking. | `not-a-cons-migration-target` | Already migrated away from cons shape. |
