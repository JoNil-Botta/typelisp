    ...
    fn lower_expr(&mut self, expr: &ast::Expr) -> Value {
        match expr.unspan() {
            ...
            ast::Expr::Comptime { expr } => match CtfeEvaluator::new().eval(expr) {
                ...
                Ok(CtfeValue::Type(_)) => {
                    unreachable!("type values should be rejected by the typechecker")
                }
                Err(_) => unreachable!("comptime should be evaluated by the typechecker"),
            },
            ast::Expr::TypeLiteral { .. } => {
                unreachable!("type literals should be rejected by the typechecker")
            }
