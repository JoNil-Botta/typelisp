...
/// Append generated-declaration provenance to a duplicate-name diagnostic.
/// For ordinary parsed declarations this adds nothing, preserving the existing
/// message; when either the existing or conflicting declaration came from a
/// `(comptime-decl ...)` template, it records which side was generated and the
/// generator's span so the error names the comptime declaration responsible.
fn append_generated_provenance(msg: &mut String, prev: &DeclOrigin, cur: &DeclOrigin) {
    if let DeclOrigin::Generated(g) = prev {
        msg.push_str(&format!(
            "; the existing definition was generated by a comptime-decl at {:?}",
            g.generator_span
        ));
    }
    if let DeclOrigin::Generated(g) = cur {
        msg.push_str(&format!(
            "; the conflicting definition was generated by a comptime-decl at {:?}",
            g.generator_span
        ));
    }
}
...
impl TypeChecker {
    ...
    pub fn check_expanded_program(&mut self, prog: &ExpandedProgram) -> Result<(), TypeError> {
        ...
        let mut seen_values: HashMap<String, DeclOrigin> = HashMap::new();
        let mut seen_types: HashMap<String, DeclOrigin> = HashMap::new();
        for expanded in &prog.decls {
            ...
            if let Some((name, span)) = type_name {
                if let Some(prev) = seen_types.get(&name) {
                    let mut msg = format!(
                        "duplicate top-level type name '{}' (already defined at {:?})",
                        name,
                        prev.diagnostic_span(),
                    );
                    append_generated_provenance(&mut msg, prev, &expanded.origin);
                    return Err(TypeError::at(msg, span));
                }
                seen_types.insert(name, expanded.origin.clone());
            }
            ...
            let value_names: Vec<(String, Span)> = match decl {
                ...
                // Imports are stripped before typecheck; `comptime-decl`
            ...
            for (name, span) in value_names {
                ...
                if let Some(prev) = seen_values.get(&name) {
                    let mut msg = format!(
                ...
                seen_values.insert(name, expanded.origin.clone());
            }
        }
    ...
    fn check_expr(&mut self, expr: &Expr) -> Result<Type, TypeError> {
        ...
        match expr.unspan() {
            ...
            Expr::Cast { expr, ty } => {
                ...
                // Casts are defined across the full scalar numeric matrix:
                // integers, `char` (an 8-bit code unit here), and the floats
                // `f32`/`f64`. Any pair of these is convertible — integer
                // widen/narrow/sign-change, `f32 <-> f64` precision changes,
                // and `int <-> float` conversions (the backend emits the
                // `cvtsi2s*`/`cvtts*2si`/`cvts*2s*` instruction families).
                let numeric =
                    |t: &Type| t.is_integer() || matches!(t, Type::Char | Type::F32 | Type::F64);
                if !numeric(&expr_ty) || !numeric(&ty) {
                    return Err(TypeError::at(
                        format!(
                            "cast requires scalar numeric (integer/char/float) source and target, got {} -> {}",
                            expr_ty, ty
                        ),
                        span,
                    ));
...
mod tests {
    ...
    #[test]
    fn test_typecheck_comptime_decl_struct_usable() {
        // A `(comptime-decl (defstruct ...))` template synthesizes a named
        // struct usable by later annotations, its constructor, and struct-get.
        let prog = parse(
            r#"
    ...
    #[test]
    fn test_typecheck_comptime_decl_enum_usable() {
        // A `(comptime-decl (defenum ...))` template synthesizes a named enum
        // usable by variant constructors and `match`.
        let prog = parse(
            r#"
            (comptime-decl (defenum Maybe (Just i64) (Nothing)))
            (define (main) : i64
              (match (Just 5) [(Just n) n] [(Nothing) 0]))
            "#,
        )
        .unwrap();
        let mut tc = TypeChecker::new();
    ...
    #[test]
    fn test_typecheck_comptime_decl_duplicate_with_parsed_type_error() {
        // A generated type name that collides with a parsed declaration is
        // caught by the same duplicate check, with generated provenance.
        let src = r#"
            (defstruct Point (x i64))
            (comptime-decl (defstruct Point (y i64)))
        "#;
        let prog = parse(src).unwrap();
        let mut tc = TypeChecker::new();
        let err = tc.check_program(&prog).unwrap_err();
        assert!(
            err.msg.contains("duplicate top-level type name 'Point'"),
            "err: {}",
            err
        );
        assert!(
            err.msg.contains("generated by a comptime-decl"),
            "expected generated provenance, err: {}",
            err
        );
    }
    ...
    #[test]
    fn test_typecheck_float_to_int_cast_is_allowed() {
        // Float -> integer casts truncate toward zero and are part of the full
        // numeric cast matrix.
        assert!(check("(define (main) : i64 (cast 3.5 : i64))").is_ok());
    }

    #[test]
    fn test_typecheck_int_to_float_cast_is_allowed() {
        assert!(check("(define (main) : f64 (cast 5 : f64))").is_ok());
        assert!(check("(define (to-f32 [n : i32]) : f32 (cast n : f32))").is_ok());
    }
    ...
    fn test_typecheck_non_float_cast_diagnostic_stays_generic() {
        ...
        assert_eq!(
            err.msg,
            "cast requires scalar numeric (integer/char/float) source and target, got bool -> i64"
        );
    }
