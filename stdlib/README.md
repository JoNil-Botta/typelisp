...
| `(vector T)` generated modules in `vector.tl` | Vector constructors, growth, `push`, `from-array`, `extend`, `to-array`, and `map*` allocate backing arrays in the active arena. `map*` traverses borrowed `Vec` values and returns fresh owned vectors; borrowed slice traversal remains separate in `vector_slice.tl`. `set` and `reverse!` mutate through `&mut`; `get`, `last`, `len`, `capacity`, `is-empty?`, `contains?`, and `fold*` are non-allocating aside from caller-provided fallback/value/function storage. |
