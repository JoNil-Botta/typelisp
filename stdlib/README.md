...
- `vector_slice.tl`: lifetime-scoped typed slice views generated by the
  `(slice T)` module macro over the matching `(vector T)` module. Import the module with
  ...
  `Slice`, mutable `MutSlice`, `from-vec`, `all-vec`, `from-vec-mut`,
  `all-vec-mut`, `get`, `set`, `len`, `mut-len`, `is-empty?`,
  `mut-is-empty?`, `sub-slice`, `to-vec`,
  and value-threaded iterator helpers. Generated slice fields `slots`, `start`,
  and `len` support inferred backing borrows before a `foreach` body while
  keeping public signatures on vector/slice views. Invalid ranges return empty
  views, and `to-vec` is the explicit owned-copy boundary.
