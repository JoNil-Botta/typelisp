...
- `math.tl`: pure scalar math helpers with no runtime imports or platform
  ...
  power-of-two scaling. Bit helpers preserve NaN payloads and signed zero and
  lower without allocation or runtime calls. Transcendental/libm-style
...
| `math-*` helpers | Pure scalar arithmetic/comparison and IEEE-754 helpers are non-allocating and import no runtime or platform externs. Typed bit reinterpretation preserves every `f64`/`f32` bit, classification distinguishes finite/infinity/NaN/normal/subnormal/signed zero, copy-sign preserves zero and NaN signs, and `f64-scalbn` / `f32-scalbn` scale across normal/subnormal/overflow boundaries. The `abs` macro covers typed `f64` and `f32` expressions; `i64` uses `math-i64-abs`/`math-i64-abs-or` for explicit signed-min behavior. Transcendental functions land as separate freestanding slices. |
