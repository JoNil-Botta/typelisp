...
- `atomic.tl`: owned `AtomicI32Buffer` and `AtomicI64Buffer` domains with
  ...
  backing storage is private dynamic-buffer storage, so callers never declare
  an unsized array. This is the only safe overlap-tolerant SPMD scatter write
  path; ordinary array-element place assignment remains non-atomic. Import it with
...
- `byte_buf.tl`: owned mutable binary byte buffers backed by private
  active-arena dynamic-buffer storage. The public API covers construction,
  ...
  sub-slices. It does not expose private dynamic buffers.
...
- `format.tl`: Rust-style literal `format` macro returning a fresh `String`.
  It accepts implicit `{}`, indexed `{0}`, explicit named `(format.named name
  value)`, and call-site-captured `{name}` selections, escaped `{{` / `}}`
  ...
  `b`, `o`, `x`, and `X` selectors render binary, octal, and lower/uppercase
  hexadecimal;
  negative signed values retain their exact 8/16/32/64-bit two's-complement
  pattern, and `#` supplies `0b`/`0o`/`0x`. The `e`
  and `E` selectors accept all eight integer widths plus f32/f64. They emit one
  leading decimal digit and a minimal signed exponent, use source-width
  shortest float digits when precision is omitted, and use exact half-even
  significant-digit rounding when precision is present. `E` changes only the
  marker; `#` is a documented no-op. Width, fill, alignment, sign, and zero
  padding use the common numeric path, while signed zero and signless canonical
  `NaN` are preserved. The `p` selector accepts direct `(Ptr T)` / `(MutPtr T)`
  values and shared borrows of those values. It emits minimal lowercase `0x`
  address text; `#p` defaults to
...
- `env.tl`: recoverable environment variable lookup/mutation and PATH-style
  ...
  `set!`, `unset!`, and target-cfg-derived `path-separator` wrappers. Lookups
  are implemented entirely in TypeLisp (#2142 follow-up): the Linux side walks
  the current SysV `envp` array and the Windows side scans the
  `GetEnvironmentStringsA` block case-insensitively. `set!` preserves present
  empty values; `unset!` succeeds for missing variables. Both return `false`
...
- `cpu.tl`: host CPU SIMD ISA detection via stdlib-owned `cpuid`/`xgetbv`
  ...
  checks.
- `core_macros.tl`: typed expression macros for core guards, booleans, and
  scalar iteration.
  The compile driver imports it as an implicit prelude, so bare `when`,
  `unless`, `and`, `or`, bracket-arm `cond`, and let-like `for` are available
  without imports. `for` selects shared, mutable, or consuming iterator
  protocol functions from the source ownership shape and zips multiple
  bindings to the shortest input.
...
- `set.tl`: module-emitting `(set T)` macro over the same open-addressing
  ...
  `contains?` / `len` / `capacity`, place-taking `insert!` / `remove!`,
  ...
  the place-taking `remove-borrowed!` wrapper. The public macros expand to
...
- `sort.tl`: generated stable deterministic hybrid merge-sort helpers for
  ...
  aggregate element types pass an explicit less-than function. Inputs of at
  most 16 elements use allocation-free insertion sort. Larger inputs use
  O(n log n) bottom-up merging and one O(n) scratch array allocated in the
...
- `math.tl`: pure scalar math helpers with no runtime imports or platform
  ...
  sign predicates; exact `f64`/`u64` and `f32`/`u32` bit reinterpretation;
  finite/infinity/NaN/normal/subnormal/signed-zero classification; special
  ...
  square-root, exponential, and trigonometric helpers lower without
  allocation, runtime calls, libm, or CRT dependencies. Import it with
  `(import stdlib.math)`.
...
- `queue.tl`: generated growable deque family (collections v1, #1549/#2797)
  over a circular compiler-private dynamic buffer. Import `(queue.deque T)` with a module alias,
...
- `thread.tl`: minimal native thread primitives for selfhost worker pools:
  ...
  arena before returning the joined aggregate. The i64 Vec live prefix is
...
- `vector.tl`: generated concrete vector family (collections v1, #835/#1989)
  over compiler-private dynamic backing storage. Its module-emitting `(vector
  T)` macro creates
  ...
  reads, lifetime-scoped native `Slice T` views through `view` / `view-mut`,
  owned copies through `from-slice`, shared `Iter` / `IterNext` traversal, consuming `IntoIter` /
  ...
  and fold/map/contains helpers. Borrow `slots` / `slots-mut` outside
  `foreach` when SPMD code needs backing storage, and let the local borrow type
  infer rather than naming the private backing type. Use
  ...
  full iterator, clone/pop, map/fold, reverse, and contains surface
  is required. Checked fixtures cover nominal enum and struct element types,
  with `push`/`set` moving values into backing slots, `pop` moving the last
  element out, and growth helpers copying the live prefix through the same move
  semantics. Current aggregate slots use the handle representation;
  ...
  construction, `set` cleans the replaced value, yielded consuming items own
  their payload, and abandoning the consuming iterator drains every still-live
  unvisited slot exactly once. This specialization exposes the ownership-safe
  construction, access, mutation, and iterator subset; clone-dependent helpers
  are intentionally omitted. Use generated vectors for append-heavy private
  sequences and keep
  Native `slice-view` / `slice-mut-view`, `slice-split-at` /
  `slice-split-at-mut`, `array-length`, `array-ref`, and array-element place
  assignment provide sub-slicing, traversal, and mutation without another generated
  family. Recursive enum lists remain appropriate where the cons shape is the
...
| `format.format` | Parses a literal template into a deterministic plan at macro expansion and emits one flat `str-cat` expression, or materializes a retained Arguments package through the shared `Formatter` path. Implicit, indexed, explicit named, call-site-captured, and star precision value/count selections bind every supplied expression once in source order. All fixed-width integer types use direct full-range decimal conversion or one shared shift/mask `b`/`o`/`x`/`X` radix converter; same-width unsigned casts preserve negative signed bit patterns. f32/f64 use source-width exact-boundary shortest Display when precision is omitted and exact fixed-decimal round-half-to-even when precision is present. Integer/float `e` and `E` reuse those decimal foundations and a common exact-size exponent layout; present exponent precision rounds exact significant digits half to even. Nominal placeholders call the canonical owner module's exact shared-borrow display hook without moving places. An unchanged value takes the existing fast path; applying shared options allocates an option-rendered piece only when content, sign, or padding changes, then the final `String` allocates once in the active arena. Integer Display/radix conversion allocates one exact-size rendered piece; sign/prefix/width changes allocate one additional option-rendered piece, while integer exponent conversion allocates its decimal piece plus normalized output. Finite float conversion also allocates fixed-capacity active-arena bignum scratch buffers before its rendered piece. Fixed and precise-exponent float conversion use a separate wider scratch capacity sufficient for every f32/f64 exact coefficient, then perform checked one-allocation final layout. Owner `to-string` hooks may allocate their rendered pieces; a matching `format-write` direct hook avoids that allocation for default Display. |
| `format.write!`, `format.writeln!`, `io.print`, `io.println`, `io.eprint`, `io.eprintln` | Accept either a literal template plus values or one retained Arguments package. They reuse the literal scanner, selection rules, options, radix/exponent conversions, and canonical nominal hook lookup from `format.format`, but send each plan piece through one `Formatter` raw callback instead of materializing the final combined String. `writeln!`, `println`, and `eprintln` append exactly one newline after a successful body. Writer calls return `FormatOk` or the first `FormatErr status` and skip later pieces after failure. ByteBuf copies into reserved backing storage, TextBuf copies retained chunks, FileHandle retries through the host writer, and stdout/stderr differ only by callback. An owner module may define `format-write` or `format-write-<NominalName>` with exact signature `(& T, &mut Formatter) -> unit`; default Display uses it for every destination, while option-bearing placeholders consistently fall back to `to-string`. Scalar conversion, option rendering, retained TextBuf chunks, and fallback `to-string` hooks keep their existing active-arena allocation behavior. `print-format` is a migration alias for `print`. |
