...
- `math.tl`: pure scalar math helpers with no runtime imports or platform
  ...
  operand precision. Freestanding `f64-log` and `f32-log` use deterministic
  bit decomposition, table reduction, and fixed-order polynomial evaluation
  within one ULP; the one-evaluation `log` macro preserves operand precision.
  Full-range `f64-sin` / `f64-cos` / `f64-tan` and
  ...
  square-root, exponential, logarithm, and trigonometric helpers lower without
...
| `math-*` helpers | Pure scalar arithmetic/comparison and IEEE-754 helpers are non-allocating and import no runtime or platform externs. Typed bit reinterpretation preserves every `f64`/`f32` bit, classification distinguishes finite/infinity/NaN/normal/subnormal/signed zero, copy-sign preserves zero and NaN signs, and `f64-scalbn` / `f32-scalbn` scale across normal/subnormal/overflow boundaries. `f64-sqrt`, `f32-sqrt`, and the type-preserving one-evaluation `sqrt` macro lower directly to scalar SSE2. Typed exponential and natural-log functions plus the one-evaluation `exp` / `log` macros use deterministic table/polynomial reductions within one ULP. These paths have no allocation, runtime call, libm, or CRT dependency. The `abs` macro covers typed `f64` and `f32` expressions; `i64` uses `math-i64-abs`/`math-i64-abs-or` for explicit signed-min behavior. Remaining transcendental functions land as separate freestanding slices. |
