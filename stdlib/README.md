...
- `net/http_types.tl`, `net/http_head_codec.tl`, `net/http_trailer.tl`, and
  `net/http_trailer_policy.tl`: byte-oriented checked
  ...
  `(import stdlib.net.http_head_codec)`. The trailer policy is a closed,
  fail-safe extension-name allowlist with explicit sensitivity and multiplicity
...
- `format.tl`: Rust-style literal `format` macro returning a fresh `String`.
  ...
  round-trip. Primitive `?`, `x?`, and `X?` Debug covers unit, bool, every
  integer width, f32/f64, quoted byte String/borrowed-str/char values, checked
  references, raw pointers, and retained Arguments. Debug hex changes integer
  leaves only. General float Debug uses the Rust 1.98 `1e-4`/`1e16` thresholds;
  explicit precision reuses exact fixed half-even conversion. String/str/char
  Debug uses `\\0`/`\\t`/`\\n`/`\\r`, escapes backslash and only the active
  delimiter, and renders every other non-printable or high byte as lowercase
  fixed-width `\\xNN`; the already-quoted leaf ignores outer options. Public
  `DebugOptions` and `DebugIntegerMode` carry resolved options for later
  owner/builders. Nominal and aggregate Debug remains a focused unsupported
  type until that layer lands and never reuses Display hooks. Default Display values are
...
- `crypto_random.tl`: cryptographically secure random-byte fills over exact
  ...
  kernel32 without adding a bcrypt import. It returns structured failures,
...
- `crypto_sha1_git.tl`: self-hosted streaming SHA-1 only for traditional Git
  object IDs and pack/index checksum compatibility. SHA-1 is collision-broken
  and this module is forbidden for signatures, passwords, package
  authenticity, TLS, new MACs, and new protocols. Its `GitSha1*` types,
  `git-digest` one-shot adapter, checked `< 2^64 bits` counter, and exact
  20-byte consuming raw sink make the compatibility boundary explicit while
  leaving one cleanup-capable seam for the separately restricted OpenSSH
...
- `random.tl`: deterministic, seeded, non-cryptographic random helpers,
  ...
  deterministic MINSTD generator and is forbidden for keys, nonces,
  challenges, blinding values, and protocol secrets. Import it with
...
Run `scripts/verify-embedded-stdlib-payload.sh` to validate all 55 explicit
