...
| `math-*` helpers | Pure scalar arithmetic/comparison and IEEE-754 helpers are non-allocating and import no runtime or platform externs. Typed bit reinterpretation preserves every `f64`/`f32` bit, classification distinguishes finite/infinity/NaN/normal/subnormal/signed zero, copy-sign preserves zero and NaN signs, and `f64-scalbn` / `f32-scalbn` scale across normal/subnormal/overflow boundaries. `f64-sqrt`, `f32-sqrt`, and the type-preserving one-evaluation `sqrt` macro lower directly to scalar SSE2 with no allocation, runtime call, libm, or CRT dependency. The `abs` macro covers typed `f64` and `f32` expressions; `i64` uses `math-i64-abs`/`math-i64-abs-or` for explicit signed-min behavior. Remaining transcendental functions land as separate freestanding slices. |
