...
- `format.tl`: Rust-style literal `format` macro returning a fresh `String`.
  ...
  semantics diagnostics. Supported values are `String`, all eight fixed-width
  integer types, `bool`, `char`, `f64`, and `f32`. Integers use full-range
  decimal Display; floats use source-width shortest round-tripping decimal
  Display without exponent notation and with canonical `NaN` spelling. Every
  supplied expression is evaluated once in source order. A nominal struct or
...
- `array.tl`: public fixed-array helpers plus transitional qualified
  dynamic-buffer macros that expand to compiler-private intrinsics during the
  #3421 migration. Import it with `(import stdlib.array)` when code must use
  that qualified stdlib-owned transition surface; bare dynamic-array aliases
  are rejected.
...
- `vector.tl`: generated concrete vector family (collections v1, #835/#1989)
  ...
  `slice-split-at-mut`, `array-length`, `array-ref`, and
  `array-set!` provide sub-slicing and traversal without another generated
...
| `format.format` | Parses a literal template into a deterministic plan at macro expansion and emits one flat `str-cat` expression. Implicit, indexed, explicit named, call-site-captured, and star precision value/count selections bind every supplied expression once in source order. All fixed-width integer types use direct decimal conversion, including full-range `u64`; f32/f64 use separate exact-boundary shortest conversion and decimal-only Display spelling. Nominal placeholders call the canonical owner module's exact shared-borrow display hook without moving places. An unchanged value takes the existing fast path; applying shared options allocates an option-rendered piece only when content, sign, or padding changes, then the final `String` allocates once in the active arena. Integer conversion allocates one rendered piece; finite float conversion also allocates fixed-capacity active-arena bignum scratch buffers before its rendered piece. Owner hooks may allocate their rendered pieces before those steps. |
...
| `array.tl` helper macros | Transitional qualified macro wrappers add no allocation by themselves. `array.make-array` allocates and initializes compiler-private dynamic-buffer storage, `array.array-length` and `array.array-ref` inspect existing storage, `array.array-set!` mutates it, and `array.array-push!` may grow it. Bare `make-array` and `array-push!` are not public language forms. |
