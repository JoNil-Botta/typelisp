...
- `sync.tl`: semaphore-backed synchronization helpers over `thread.tl`.
  ...
  in runtime-owned OS memory; `ChannelI64PairChannel` is the first move-based
  aggregate channel surface for two-field `ChannelI64Pair` messages; `MutexI64`
  protects a shared scalar through a cleanup-owned lexical guard and rejects
  close while guards or lock attempts are live. Import it with
...
- `thread.tl`: minimal native thread primitives for selfhost worker pools:
  ...
  `thread-spawn-string`/`thread-join-string` run the task in a fresh atomic
  arena before returning the joined `String`. Linux uses raw clone/futex/eventfd
...
| `ChannelI64` and `ChannelI64PairChannel` helpers in `sync.tl` | Channel creation allocates runtime-owned OS memory for the fixed ring buffer and head/tail state, plus three OS semaphore handles. Send/recv do not allocate; they block through the semaphore substrate and move one scalar `i64` message or one two-`i64` `ChannelI64Pair` aggregate through the synchronized queue. `channel-i64-close` and `channel-i64-pair-close` release the OS memory and semaphore handles after all users are done. |
...
| `thread-*` helpers in `thread.tl` | Thread spawning allocates a small active-arena context, join/result cells, and on Linux a raw worker stack before the OS thread starts. Each worker initializes a fresh per-thread default arena before calling user code. `thread-spawn-string` also allocates a fresh atomic arena and one result cell so the joined `String` storage can safely outlive the worker. Semaphore handles are OS resources and do not allocate TypeLisp heap storage beyond result aggregates. The raw `i64` context/result surface still does not transfer ownership; callers that pass addresses through it remain responsible for synchronization in unsafe code. |
