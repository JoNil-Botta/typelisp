...
- `http_trailer.tl` covers bounded incremental response-trailer parsing,
  normalized declaration provenance, shared eligibility/sensitivity and all
  multiplicities, exact suffix consumption, every valid and failing input
  split, stable terminal/EOF outcomes, and independent wire/retained limits.
...
- `math.tl` owns inline tests for the pure scalar helpers for `i64`, `f64`,
  ...
  `tests/integration/stdlib_math_log.tl` checks MPFR-256 curated and sampled
  vectors plus the complete logarithm special-value contract on both targets.
  ...
  and NaNs.
  `tests/integration/stdlib_math_exp.tl` checks both exponential entry points
