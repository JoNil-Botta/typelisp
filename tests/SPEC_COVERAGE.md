...
| S2-005 | 2.4 | Integer literals default to `i64`, adopt expected integer types with range checks, and float literals default to `f64` with contextual `f32` support. | Implemented | `src/tests/compiler_typecheck_smoke.tl`, `tests/integration/arithmetic.tl`, `tests/integration/f32_scalar.tl`, `tests/integration/overflow_casts.tl`, `tests/safety/integer_wrap_cast_defined.tl`, `tests/safety/integer_literal_*_reject.tl` | Covered | - |
...
| S3-003 | 3.2, 3.11 | Bare `str` and `bytes` are not by-value types; borrowed text uses `(& r str)`. | `str` implemented; `bytes` pending | `stdlib/string.tl` inline tests, `stdlib/string_caller_result.tl` inline tests, `tests/safety/runtime_type_literal_reject.tl` | Partial | #2783 |
...
| S3-019 | 3.11 | `ByteBuf`, `(& r bytes)`, and `(&mut r bytes)` binary storage contract and conversions. | Partial: borrowed `bytes` referent syntax/type policy is implemented; `ByteBuf` owner/conversions remain pending | `src/tests/compiler_typecheck_smoke.tl` borrowed `bytes` positive and bare-value rejection tests; compatibility byte paths still use `String` or `(Array u8)` | Partial | #2783, #2784 |
...
| S5-012 | 5.15 | `foreach` scalar semantics, contiguous map/zip lowering, tail masks, supported lane types, uniform/varying rules, public vector/mask/varying source type deferral, and SPMD safety rejections. | Implemented for current slice | `tests/integration/spmd_foreach.tl`, `tests/spmd/*.tl`, `scripts/verify-spmd-simd.sh`, `tests/safety/spmd_*_reject.tl`, `tests/safety/spmd_public_vector_type_reject.tl`, `tests/safety/spmd_public_mask_type_reject.tl`, `tests/safety/spmd_varying_type_reject.tl` | Covered | - |
...
| S6-005 | 6.5 | Safe task threading, structural transfer/share classification, safe spawn/join, mutex guards, channels, and atomics. | Partial: raw substrate, reusable structural transfer/share classifier for safe-spawn captures, concrete checked `thread-spawn-*`/`thread-join-*` wrappers for `i64`, `bool`, and `unit`, concrete `ChannelI64`, and concrete `MutexI64` lexical guards are present; aggregate channel transfer, aggregate typed joins, and atomic-arena-backed typed returns remain pending | `tests/integration/thread_runtime.tl`, `tests/integration/thread_safe_i64.tl`, `tests/integration/thread_safe_bool_unit.tl`, `tests/integration/thread_safe_aggregate_capture.tl`, `tests/integration/sync_channel_thread_runtime.tl`, `tests/integration/sync_mutex_thread_runtime.tl`, `stdlib/tests/thread_api.tl`, `stdlib/tests/sync_api.tl`, `tests/safety/thread_spawn_*`, `tests/safety/thread_join_*`, and `tests/safety/sync_mutex_guard_*_reject.tl` cover current raw/std surfaces | Partial | #2714, #2715, #2716, #2717 |
...
| S7-004 | 7.3 | Lifetime owner classes and lexical outlives model constrain references, regions, first-class arenas, and atomic arenas. | Lexical references/regions, ordinary first-class arena helpers, and `in-arena` are implemented; atomic arena slices pending | Reference/arena safety corpus, `src/tests/compiler_typecheck_smoke.tl`, `tests/integration/in_arena.tl`, `tests/safety/in_arena_inner_region_escape.tl`, `stdlib/arena.tl` inline tests, `stdlib/string.tl` inline tests, `stdlib/text_buf.tl` inline tests, `stdlib/tests/arena_patterns.tl`, `stdlib/tests/arena_policy_escape_*.tl` | Partial | #2642 |
...
| S8-005 | 8.2 | Unsupported SPMD features remain negative/diagnostic work: public lane ids, gathers/scatters, cross-lane ops, atomics, public vector/mask values, varying control-flow leftovers, varying calls, unsupported lane values. | Public vector/mask/varying source type deferral is pinned; remaining deferred features are partially covered by negative safety corpus | `tests/safety/spmd_*_reject.tl`, `tests/safety/spmd_masked_if_*_reject.tl`, `tests/safety/spmd_public_vector_type_reject.tl`, `tests/safety/spmd_public_mask_type_reject.tl`, `tests/safety/spmd_varying_type_reject.tl` | Partial | #2761, #2762, #2764, #2765, #2766, #2768, #2690 |
