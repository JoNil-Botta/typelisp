...
| ID | SPEC section | Normative behavior | Implementation status | Coverage owner/test(s) | Gap status | Follow-up |
| --- | --- | --- | --- | --- | --- | --- |
| S4-001 | 4.1 | Global `define` accepts constant scalar initializers and rejects unsupported aggregate globals unless covered by current runtime initializer implementation. | Implemented, with runtime initializer extensions beyond older wording | `tests/integration/global_initializer.tl`, `tests/integration/aggregate_globals.tl`, parser/typechecker smokes | Partial | #2687 |
...
| S4-003 | 4.3 | Extern forms, exact symbol/link metadata, function pointers, varargs markers, link input dedup, and ABI-supported signatures. | Implemented | C ABI fixtures, `tests/safety/foreign_abi_metadata_reject.tl`, `stdlib/tests/ffi_api.tl`, public-tool build/link coverage | Partial | #2687 |
| S4-004 | 4.3.1 | `(unsafe declaration)` gates safe calls/references to unsafe functions and externs. | Implemented | `tests/safety/safe_raw_pointer_read_reject.tl`, `tests/safety/foreign_abi_metadata_reject.tl`, raw-pointer fixtures | Partial | #2687 |
...
| S4-006 | 4.4.1-4.4.4 | Dotted module identities, aliases, exports/visibility, qualified lookup, and macro export/import ordering. | Partially implemented; repository migration ongoing | `tests/integration/implicit_core_prelude.tl`, public-tool LSP/import fixtures, parser/typechecker smoke tests | GAP | #2687, #2452, #2453, #2454, #2492 |
| S4-007 | 4.4.5 | TypeLisp linker symbol identity uses canonical module identity and declaration identity; extern/runtime symbols stay exact. | Implemented in current backend symbol model | `selfhost/compiler_backend_smoke.tl`, `selfhost/compile_manifest.txt`, C ABI fixtures | Partial | #2687 |
| S4-008 | 4.4.6 | `include-str` embeds UTF-8 text with import-like path resolution and diagnostics. | Implemented parser/loader surface | Parser/load smoke tests | GAP | #2687 |
| S4-009 | 4.4.7 | Top-level and expression `cfg` predicates support names, `all`, `any`, `not`, target OS predicates, and inactive-branch parsing rules. | Implemented | `scripts/check-codegen-target-parity.sh`, public-tool compile coverage, selfhost parser/load smoke tests | GAP | #2687 |
| S4-010 | 4.5 | Inline `(test ...)` items are ignored by production commands and owned by `typelisp test` harness generation. | Implemented | `scripts/verify-inline-tests.sh`, `tests/inline/*.tl`, `stdlib/tests/*`, `selfhost/test_cli_core.tl` | Covered | - |
| S4-011 | 4.6 package | `typelisp.pkg` manifests, kind/entry defaults, local and git dependencies, lockfile replay/update, package cache, DAG builds, and package output paths. | Implemented | `scripts/verify-public-tools.sh`, package smoke tests, `selfhost/package_lock_smoke.tl`, `selfhost/package_cache_smoke.tl`, `selfhost/build_cli_core.tl` tests | Partial | #2687 |
| S4-012 | 4.6 defenum/defstruct | Top-level `defenum` and `defstruct` behavior delegates to the type-system rules. | Implemented | Enum/struct integration fixtures and parser/typechecker smokes | Covered | - |
| S4-013 | 4.6.1 | Cleanup-owning struct declarations, field cleanup metadata, generated cleanup function, LIFO field cleanup, and escape/copy restrictions. | Implemented for structs; cleanup-owning enums reserved | `with` and cleanup checker smoke tests, resource cleanup fixtures | GAP | #2687 |
| S4-014 | 4.6.2 | Move-only aggregate handle checking, path moves/reinitialization, non-consuming inspection, loop conservatism, and diagnostics. | Implemented | `tests/integration/clone_deep_copy.tl`, `tests/integration/array_push_aggregate.tl`, `tests/safety/auto_borrow_*_reject.tl`, selfhost typechecker smokes | Partial | #2687 |
| S4-015 | 4.6.3 | Recursive inline aggregate cycles require explicit indirection; finite-layout analysis rejects unboxed cycles. | Partially implemented | Selfhost finite-layout/typechecker tests; SPEC examples are ignored until full enforcement | GAP | #2687 |
...
| S5-011 | 5.14 | Lambdas cover non-capturing lowering, capturing environments, aggregate captures/returns, fixed-array captures, and mutable-capture rejection. | Implemented except mutable captures remain rejected/policy pending | `tests/integration/lambda_*.tl`, `tests/integration/tail_indirect_closure.tl` | Partial | #2688, #2552 |
...
| S6-002 | 6.2 | Backend emits allocator, arena, string, and trap runtime helpers without a separate C runtime; Linux/Windows page paths and TLS/current-arena policy are target-owned. | Implemented for current targets | `tests/integration/tl_alloc.tl`, `tests/integration/tl_alloc_huge_trap.tl`, `tests/integration/region_*.tl`, native link scripts, backend smokes | Partial | #2689 |
...
| ID | SPEC section | Normative behavior | Implementation status | Coverage owner/test(s) | Gap status | Follow-up |
| --- | --- | --- | --- | --- | --- | --- |
| S7-001 | 7.1 | Stack slots, downward stack growth, compile-time frame sizing, and 16-byte call alignment. | Implemented | `tests/integration/many_args.tl`, `tests/integration/register_scalar_homes.tl`, backend smoke tests, C ABI fixtures | Partial | #2689 |
...
| S7-004 | 7.3 | Lifetime owner classes and lexical outlives model constrain references, regions, first-class arenas, and atomic arenas. | Partially implemented; atomic and safe thread crossing staged | Reference/arena safety corpus, stdlib arena tests | Partial | #2689, #2625, #2642 |
...
| S7-008 | 7.5 | Globals live in data/rodata, string literal bytes are rodata, and `String` points at inline ptr/len storage. | Implemented | `tests/integration/global_initializer.tl`, `tests/integration/aggregate_globals.tl`, `tests/golden/*.s`, backend smokes | Partial | #2689 |
| S7-009 | 7.6 | Aggregate handles are move-only source values, non-consuming inspection is compatibility borrow-like behavior, mutation requires owned/mutable access, returns may heap-promote, and `clone` deep-copies supported shapes. | Implemented for current checker/lowering | `tests/integration/clone_deep_copy.tl`, `tests/integration/array_push_aggregate.tl`, `tests/integration/register_resident_*`, `tests/safety/struct_field_set_*_reject.tl` | Partial | #2689 |
...
| S8-003 | 8.2 | Mutable lambda captures are not implemented or must be formally rejected. | Not implemented / policy pending | No dedicated negative row in matrix yet | GAP | #2552, #2690 |
...
| ID | SPEC section | Normative behavior | Implementation status | Coverage owner/test(s) | Gap status | Follow-up |
| --- | --- | --- | --- | --- | --- | --- |
| S10-001 | 10 | Root command list and common options expose `build`, `check`, `compile`, `doc`, `fmt`, `init`, `lint`, `lsp`, `new`, `repl`, `run`, and `test`. | Implemented | `tests/public-tools/cli-command-surface.txt`, `scripts/verify-public-tools.sh`, selfhost command wrappers in compile manifest | Covered | - |
