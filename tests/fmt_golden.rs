...
fn fmt_produces_golden_output_and_is_idempotent() {
    if cfg!(any(target_os = "linux", target_os = "windows")) {
        ...
        let eval = manifest_dir.join("selfhost").join("eval.tl");
        let eval_arg = eval.to_str().expect("selfhost eval path is utf-8");
        let check_eval = run(&["fmt", "--check", eval_arg]);
        assert!(
            check_eval.status.success(),
            "fmt --check selfhost/eval.tl failed\nstatus: {}\nstderr:\n{}",
            check_eval.status,
            String::from_utf8_lossy(&check_eval.stderr),
        );
    }
}
