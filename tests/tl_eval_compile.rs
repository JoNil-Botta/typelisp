...
//! front end (#27): a tiny tree-walking interpreter with VARIABLES (`let`),
//! CONDITIONALS (`if`), and COMPARISON operators. Where the lexer turns a
...
//! SCons)`, the evaluator INTERPRETS that tree WITH RESPECT TO a lexical
//! environment - it walks the s-expression, dispatches on the head symbol,
//! resolves a bare symbol as a variable reference via `lookup` over the
//! cons-cell assoc-list `Env` (`ENil | (EBind String i64 Env)`), handles
//! `(let ((x e1)) body)` by pushing an `EBind` frame, handles `(if cond then
//! else)` by short-circuiting to the taken branch, and otherwise recursively
//! evaluates the argument sub-exprs for arithmetic (`+ - * /`) and comparison
//! (`= < > <= >=`) operators, computing the integer the expression denotes.
//! It does NOT
