...
#[test]
fn tl_format_rules_tl_compiles_to_assembly() {
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let source_path = manifest_dir.join("selfhost").join("format_rules.tl");

    let work_dir = manifest_dir
        .join("target")
        .join("tl-format-rules-compile-test");
    fs::create_dir_all(&work_dir).expect("create format_rules compile test work dir");
    let asm_path = work_dir.join("format_rules.s");
    ...
    assert!(
        output.status.success(),
        "format_rules.tl compile step failed\nstdout:\n{}\nstderr:\n{}",
        stdout,
        stderr,
    );

    let asm = fs::read_to_string(&asm_path).expect("read generated format_rules assembly");

    assert!(
        !asm.contains("# TODO"),
        "format_rules assembly still contains a # TODO marker:\n{}",
        asm,
    );

    for sym in [
        "_tl_format_rules_node_symbol_question:",
        "_tl_format_rules_define_doc:",
        "_tl_format_rules_special_list_doc:",
        "_tl_format_rules_cst_doc:",
        "_tl_format_rules_render_source_width:",
        "_tl_format_rules_self_test:",
        "_tl_parse_format_source:",
        "_tl_render_doc_width:",
        "tl_string_eq:",
        "tl_alloc:",
        ...
        assert!(
            asm.contains(sym),
            "format_rules assembly is missing expected symbol {}:\n{}",
            sym,
            asm,
        );
    }
    ...
    for literal in [
        ...
        assert!(
            asm.contains(literal),
            "format_rules assembly is missing expected literal {:?}:\n{}",
            literal,
            asm,
        );
    }
